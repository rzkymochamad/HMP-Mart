package com.mg.shopping.customutil;
...
public class ExtensiblePageIndicator extends View implements ViewPager.OnPageChangeListener {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < circleCount; i++) {
            ...
            canvas.drawCircle(center, getPaddingTop() + (float) circleRadius, circleRadius, inactivePaint);
        }
    ...
    private void drawRect(Canvas canvas) {
        ...
        float moveDistance = circleRadius * 2 + (float)circlePadding;
    ...
    private float getCirclePadding(int position) {
        return circlePadding * position + (float) circleRadius * 2 * position;
    }
    ...
    private float getStartedX() {
        switch (mGravity) {
            ...
            case Gravity.CENTER:
            default:
                return ( (float)  getMeasuredWidth() / 2 - getAllCirclesWidth() / 2 );
        }
    }

    private float getAllCirclesWidth() {
        return circleRadius * 2 * circleCount + (float)(circleCount - 1) * circlePadding;
    }
