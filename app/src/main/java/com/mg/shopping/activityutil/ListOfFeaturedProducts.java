...
import com.mg.shopping.adapterutil.FeaturedProductAdapter;
import com.mg.shopping.constantutil.Constant;
import com.mg.shopping.interfaceutil.SelectionInterface;
import com.mg.shopping.jsonutil.featuredproductutil.ListOfDatum;
...
public class ListOfFeaturedProducts extends BaseActivity {
    ImageView imageBack;
    StaggeredGridLayoutManager layoutManager;
    RecyclerView recyclerViewProduct;
    FeaturedProductAdapter dataAdapter;
    ...
    protected int getContentView() {
        return R.layout.activity_featured_product;
    }
    ...
    protected void initUI() {
        ...
        txtMenu.setText(Utility.getStringFromRes(context, R.string.featured_products));

        layoutManager = new StaggeredGridLayoutManager(2, LinearLayoutManager.VERTICAL);
        recyclerViewProduct = (RecyclerView) findViewById(R.id.recycler_view_product);
        recyclerViewProduct.setLayoutManager(layoutManager);

        dataAdapter = new FeaturedProductAdapter(context, objectArrayList, new SelectionInterface() {
            @Override
            public void onSelection(SelectionObject selectionObject) {
                ...
                if (selectionObject.getAction().equalsIgnoreCase("product_cart")) {
                    ...
                    com.mg.shopping.jsonutil.listofproductutil.ListOfDatum
                            listOfDatum = new com.mg.shopping.jsonutil.listofproductutil.ListOfDatum();

                    listOfDatum.setId(productDatum.getId());
                    listOfDatum.setName(productDatum.getName());
                    listOfDatum.setRating(productDatum.getRating());

                    Intent intent = new Intent(context, ProductDetail.class);
                    intent.putExtra(Constant.IntentKey.DATA_OBJECT, listOfDatum);
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                if (dy > 0) {
                    ...
                    if (!loading && totalItemCount <= (lastVisibleItem + visibleThreshold)) {
                        ...
                        sendRequestToServer(new RequestObject()
                                .setJson(getJson(new JsonObject()
                                        .setFunctionality("retrieve_featured_banner_products")
                                        .setSkipIds(skipIds)
                                        .setConnection(Constant.CONNECTION.RETRIEVE_FEATURED_PRODUCTS)
    ...
    protected String getJson(JsonObject jsObject) {
        ...
        try {
            ...
            if (jsObject.getConnection() == Constant.CONNECTION.RETRIEVE_FEATURED_PRODUCTS) {

                jsonObject.accumulate("functionality", jsObject.getFunctionality());
                jsonObject.accumulate("banner_id", jsObject.getId());
    ...
    public void onSuccess(Object data, RequestObject requestObject) {
        ...
        if (requestObject.getConnection() == Constant.CONNECTION.RETRIEVE_FEATURED_PRODUCTS) {
